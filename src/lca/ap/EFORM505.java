...
public class EFORM505 extends POI_EXCEL {
	...
	public ArrayList<Object []> getEFORM5_1_data(){
		...
		try {
			String sql = " SELECT e0.CTY, e0.KCNT, e1.RANGE, NVL(e1.CNT, 0) CNT_1, NVL(e2.CNT, 0) CNT_2 "
					...
					+ " WHERE RPT_FLAG = '2' AND SEX = '1' AND RANGE = ' ') e1, "
					+ " (SELECT CTY, RANGE, CNT FROM EFORM5_1 "
					+ " WHERE RPT_FLAG = '2' AND SEX = '2' AND RANGE = ' ') e2 "
					+ " WHERE 1=1 "
					+ " AND e0.CTY = e1.CTY(+) "
					+ " AND e0.CTY = e2.CTY(+) "
					+ " ORDER BY e0.ORDERBY ";
	...
	public ArrayList<Object []> getEFORM5_2_data(){
		...
		try {
			String sql = " SELECT e0.CTY, NVL(e1.CNT, 0) CNT1, NVL(e2.CNT, 0) CNT2, "
					...
					+ " WHERE RPT_FLAG = '2' AND SEX = '1') e1, "
					+ " (SELECT CTY, CNT, CNT1, T_AREA, T_MONEY FROM EFORM5_2 "
					+ " WHERE RPT_FLAG = '2' AND SEX = '2') e2 "
					+ " WHERE 1=1 "
					+ " AND e0.CTY = e1.CTY(+) "
					+ " AND e0.CTY = e2.CTY(+) "
					+ " ORDER BY e0.ORDERBY ";
	...
	public String doProcess(String templateURL, String workareaURL, String reportName) {
		...
		try {
			...
			*/
			...
			fillCellValue(Common.formatYYYMMDD(getLastProcessDate(), 3), 38, 0);
			fillCellValue(Common.formatYYYMMDD(getLastProcessDate(), 3), 73, 0);
			fillCellValue(Common.formatYYYMMDD(getLastProcessDate(), 3), 110, 0);
			...
			for(int i = 0; i < EFORM5_1_data.size(); i++){
				for(int j = 0; j < EFORM5_1_data.get(i).length; j++){
					fillCellValue(getBar(EFORM5_1_data.get(i)[j]), row_S+i, col_S+j);
				}
			}	
			//自動計算公式(總歸戶)
			for(int i = 5; i < 29; i++){
			...
			for(int i = 0; i < EFORM5_2_data.size(); i++){
				for(int j = 0; j < EFORM5_2_data.get(i).length; j++){
					fillCellValue(getBar(EFORM5_2_data.get(i)[j]), row_S+i, col_S+j);
				}
			}
			//自動計算公式(單一所有權人)
			for(int j = 3; j < 9; j++){ autoFormula(48, j); }  //台灣省
			for(int j = 3; j < 9; j++){ autoFormula(63, j); }  //福建省
			...
			for(int i = 0; i < EFORM5_3_data.size(); i++){
				for(int j = 0; j < EFORM5_3_data.get(i).length; j++){
					fillCellValue(getBar(EFORM5_3_data.get(i)[j]), row_S+i, col_S+j);
				}
			}
			//自動計算公式(分別共有)
			for(int j = 3; j < 9; j++){ autoFormula(83, j); }  //台灣省
			for(int j = 3; j < 9; j++){ autoFormula(98, j); }  //福建省
			...
			for(int i = 0; i < EFORM5_4_data.size(); i++){
				int j = 0;
				fillCellValue(getBar(EFORM5_4_data.get(i)[0]), row_S+i, col_S+j);
				j += 1;
				fillCellValue(getBar(EFORM5_4_data.get(i)[1]), row_S+i, col_S+j);
				j += 1;
				fillCellValue(getBar(EFORM5_4_data.get(i)[2]), row_S+i, col_S+j);
				j += 2;
				fillCellValue(getBar(EFORM5_4_data.get(i)[3]), row_S+i, col_S+j);
				j += 2;
				fillCellValue(getBar(EFORM5_4_data.get(i)[4]), row_S+i, col_S+j);
			}
			//自動計算公式(公同共有)
			autoFormula(120, 3);  //台灣省
			autoFormula(120, 5);  //台灣省
			autoFormula(120, 7);  //台灣省
			autoFormula(135, 3);  //福建省
			autoFormula(135, 5);  //福建省
			autoFormula(135, 7);  //福建省

			
			
			//自動計算公式(單一所有權人第一列)
			for(int i = 3; i < 9; i++){ autoFormula(41, i); }
			//自動計算公式(分別共有第一列)
			for(int i = 3; i < 9; i++){ autoFormula(76, i); }
			//自動計算公式(公同共有第一列)
			autoFormula(113, 3);
			autoFormula(113, 5);
			autoFormula(113, 7);
	...
	public String getCellValue(int rowNum, int cellNum){
			HSSFRow row = getHSSFSheet().getRow(rowNum);
