...
int maxPostSize = 1024*1024*10; //10 M
    ...
    if("doUpload".equals(actionType)){
        ...
        if(enu.hasMoreElements()){
            ...
            if (fileSize>0) {
                ...
                boolean delFileBol=true;//如果副檔名不符規則就將檔案刪除並回傳上傳失敗
                String[] fileType=application.getInitParameter("filestoreAllowedExtList").split(",");
                for(int i=0;i<fileType.length;i++){
                	fileType[i]="."+fileType[i];
                	//System.out.println(fileName+":"+fileName.substring(fileName.length()-fileType[i].length(), fileName.length()));
                	if(fileName.length() > fileType[i].length()){//上傳檔案的檔名小於指定的副檔名是不正常的
	                	if(fileType[i].equals(fileName.substring(fileName.length()-fileType[i].length(), fileName.length()))){
	                		delFileBol=false;
	                	}
                	}
                }
                if(delFileBol){
                    ...
                    actionMessage = "上傳失敗，上傳的檔案格式必須是"+application.getInitParameter("filestoreAllowedExtList")+"!";
                   	popFileID="";
					uploadCaseID="";
					fileName="";
                }
