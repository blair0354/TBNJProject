...
<%
String progID = util.Common.get(request.getParameter("progID"));

String sScript = request.getServletPath();
...
String programID = sScript.toLowerCase();		
String auth = "";
...
if (progID.equals("X")) {
	...
	try {
		...
		util.User user = (util.User) session.getAttribute("user");		
		if (user.getIsAdminManager().equals("Y") && user.getIsManager().equals("Y")) {
		} else {
			if ("".equals(progID)){				
				progID = (String)session.getAttribute("PID");
				if (util.Common.get(progID).equals("X")) auth = "M";
				else {
					auth = (String)session.getAttribute("PAU");
					if (auth==null || auth.equals("")) auth ="Q";
				}
			} else {
				rs = db.querySQL("select auth from sysap_authority where groupID='"+user.getGroupID()+"' and progID="+util.Common.sqlChar(progID));
				if (rs.next()) {
				...
				programID = progID;
				session.setAttribute("PID",programID);
				session.setAttribute("PAU",auth);
			}
		}
	} catch (Exception e) {
...
function getFocusField() {
	...
	if ("<%=sScript%>"=="NPBBL001F") {
		document.all.item("btnSelectOldCase").focus();
	} else {
...
<input type='hidden' name='permissionField' value='<%=auth%>'>
...
<span id="spanClose">
<input class="toolbar_default" type="button" followPK="false" name="closebtn" value="關　閉" onClick="window.close()" >&nbsp;|
</span>
