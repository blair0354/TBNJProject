...
<%
String sY = util.Common.checkGet(request.getParameter("sY"));
String sM = util.Common.checkGet(request.getParameter("sM"));
String jsFunction = "";
if (sY!=null && sY.length()==3 && sM!=null && sM.length()==2) {
	jsFunction = "drawCld('" + sY + "','" + sM + "')";
}
%>
<html>
<head>
			...
			<td> 
				<table width="150" border="0" cellspacing="2" cellpadding="0" align="center">
					<tr class="week_bg" align="center"> 
						<td class="week_day2">日</td>
						<td class="week_day1">一</td>
						<td class="week_day1">二</td>
						<td class="week_day1">三</td>
						<td class="week_day1">四</td>
						<td class="week_day1">五</td>
						<td class="week_day2">六</td>
					</tr>
					<SCRIPT language="JavaScript">
						for(i=0;i<6;i++) {
							document.write('<tr align=center>')
							for(j=0;j<7;j++) {
								document.write('<td id="GD' + (i*7+j) +'"><font id="SD' + (i*7+j) +'"  size=2 face="新細明體"')
